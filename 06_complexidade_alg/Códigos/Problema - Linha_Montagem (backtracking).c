...
/* PROBLEMA DA LINHA DE PRODUÇÃO - Backtraking e branch and bound
...
void caminha (int lin, int est, int trans, int *custo, int *min, int *saida)
{
    ...
    *custo = *custo + custos_est[lin][est];
    int aux_custo = *custo;
    if (lin==0) {
        if (*custo < *min) { cont_passo++; caminha (lin,est+1,0,custo, min, saida);  }
        if (*custo < *min) *min = *custo;
        *custo = aux_custo + custo_trans[lin][est];
        if (*custo < *min) { cont_passo++; caminha (lin+1, est+1, 1, custo, min, saida);  }
        if (*custo < *min) { *min = *custo; *saida = lin+1;    // indicando troca de saida
        }
    }
    else {
        *custo = aux_custo;
